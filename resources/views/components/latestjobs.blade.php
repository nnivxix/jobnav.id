...
<div class="grid grid-cols-3 gap-5 my-7 mx-20">
  ...
  <article class="border border-carrot-600 p-6 rounded-md hover:bg-gray-100 ">
    <div class="grid grid-cols-6 my-3">
      <img class="col-start-1 place-self-center p-3 col-end-2 row-start-1 row-end-3" src="http://localhost:8000/storage/{{ $job->company->avatar }}" alt="{{ $job->company->avatar }}" srcset="">
      <h1 class="text-smoke-700 col-start-2 col-end-6 self-end" id="company">{{ $job->company->name }}</h1>
      <h2 class="text-smoke-700 row-start-2 col-start-2 col-end-6 self-start" id="position">{{ $job->position }}</h2>
