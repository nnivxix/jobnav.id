<nav class="grid grid-cols-6 px-5 py-4">
  <h1 class="col-span-1 text-2xl font-bold"><a href="#">Jobnav.id</a></h1>
  <ul class="text-md flex w-1/2  place-self-center  col-span-4" id="middle">
    <li class="px-3"><a href="#">Jobs</a></li>
    <li class="px-3"><a href="#">Learn</a></li>
    <li class="px-3"><a href="#">Companies</a></li>
    <li class="px-3"><a href="#">Services</a></li>
  </ul>
  <ul class="text-md col-span-1 flex justify-between items-center">
    <li class="bg-orange-500 text-white py-2 px-4 rounded-lg"><a href="#">Sign up</a></li>
    <li><a href="#">Log in</a></li>
  </ul>
</nav>
